...
<body><main>
<header><div><h1>Chess Gantry Controller</h1><p class="subtitle">One local interface for Fedora serial diagnostics, direct X/Y coordinate checks, and validated chess move JSON.</p></div><div id="pill" class="pill">Starting…</div></header>
<section class="grid">
...
<div class="card"><h2>2. Machine state</h2><div class="metrics"><div class="metric"><span>X</span><strong id="xread">—</strong></div><div class="metric"><span>Y</span><strong id="yread">—</strong></div><div class="metric"><span>Homed</span><strong id="homed">No</strong></div><div class="metric"><span>Board rev.</span><strong id="revision">—</strong></div></div><div class="actions"><button id="endstops">Check endstops</button><button id="home" class="primary">Home X/Y</button><button id="stop" class="danger">Emergency stop</button></div><div class="notice">Before first homing, verify each physical endstop changes between <b>open</b> and <b>TRIGGERED</b>. Keep the controller power switch reachable.</div></div>
...
$('home').onclick=()=>{if(confirm('Have both endstops been checked? Incorrect direction or wiring can cause a crash.'))action('Homing X/Y…',()=>api('/api/home',{method:'POST',body:JSON.stringify({confirm_motion:true})}))};
