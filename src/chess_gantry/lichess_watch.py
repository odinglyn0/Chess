...
async def watch_game(
    ...
    *,
    ...
    try:
        ...
        raise ConfigurationError("websockets is not installed; run 'python -m pip install -e .'") from exc
    ...
    async with connect(endpoint, max_size=262_144) as socket:
        async for raw in socket:
            try:
                ...
                raise ValidationError(f"invalid JSON from Lichess stream: {exc}") from exc
            ...
            if kind == "move":
                if execute:
                    ...
                    move = stream_event_to_move(
                        message, service.config.board.width, service.config.board.height, state
                    )
                    move_path = output_dir / f"{move.event_id}.json"
                    move_path.write_text(json.dumps(move.to_dict(), indent=2, sort_keys=True) + "\n", encoding="ascii")
                    plan = service.execute(move)
                    (output_dir / f"{move.event_id}.gcode").write_text(plan.program.text(), encoding="ascii")
                    print(f"Executed {move.event_id}; board revision {plan.next_state.revision}.")
                else:
                ...
                if kind == "error" and message.get("fatal"):
                    raise ConfigurationError(str(message.get("message", "fatal Lichess stream error")))
                if kind == "game_over":
                    return
