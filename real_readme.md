...
<p align="center">
  ...
  Collision-aware planning, Marlin motion control, live Lichess games, and a
  secure browser dashboard.
</p>
...
<p align="center">
  ...
  <img alt="Docker Distroless" src="https://img.shields.io/badge/Runtime-Distroless-2496ED?logo=docker&logoColor=white">
</p>
...
Chess Gantry translates legal chess moves into guarded G-code for a magnetic
Cartesian gantry. It can:
...
- mirror a fresh public Lichess game with streaming move updates;
- expose authenticated local and network browser controls;
- recover safely from interrupted moves using local JSON state.
...
The physical E connector is intentionally controlled as logical Marlin Z. The
host homes with `G28 X Y Z`, waits for completion, and applies the measured
coordinate remap:
...
Open the complete token URL printed by the server from another device on the
same trusted network. Commands and serial access remain on the gantry computer.
Do not expose the plain-HTTP dashboard directly to the internet.

## Raspberry Pi 3B+

The recommended deployment is Raspberry Pi OS Lite 64-bit with Docker. The Pi
host does not need `uv`, Node.js, or a Python virtual environment. The current
image is ARM64-only; use a 64-bit Pi OS image.
...
The installer configures Docker, serial access, persistent state, a random web
token, and the authenticated dashboard.

### Container Design

```text
Build stage:  Fedora 42
Python:       Fedora system Python with uv-resolved dependencies
Runtime:      scratch-based distroless root filesystem
Platform:     linux/arm64
```

The production image contains a small `uv run` compatibility shim, Python, the
application, and runtime dependencies. It contains no shell, package manager,
Git client, Node.js, or compiler.
...
This performs a fast-forward pull, updates submodules, rebuilds the Fedora
stage, and recreates the distroless runtime. `data/`, `config.json`, and the
private `.env.docker` token file are preserved.

## Measured 64-Square Test

The primary calibration test homes the gantry, approaches h1, and visits every
square center in a serpentine path. Every adjacent move is exactly 40 mm.
...
The dashboard can follow a new public Lichess game with low latency using the
streaming game API.
...
Each web-server session creates fresh isolated board state, homes once, and
executes newly streamed plies through one persistent serial connection.

> [!IMPORTANT]
> Physical capture slots are currently disabled because the measured 40 mm grid
> leaves insufficient safe off-board storage. The live follower stops safely at
> captures or promotions instead of attempting uncalibrated motion.
...
The automated suite covers geometry, motion planning, persistence, serial
protocol, firmware configuration, Docker deployment, authentication, dashboard
task ownership, keyboard jogging, and live Lichess state.

## Safety

> [!WARNING]
> This system moves real hardware and energizes an electromagnet. Keep an
> independent physical cutoff within reach, clear every requested path, verify
> electrical flyback protection, and never leave the magnet energized
> unattended. Software checks do not replace safe mechanical and electrical
> design.
