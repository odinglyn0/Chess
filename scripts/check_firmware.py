...
from chess_gantry.errors import ChessGantryError
...
def main() -> int:
    ...
    with MarlinSerial(config.serial) as link:
        ...
        if not args.home:
            print("Firmware identity and endstop reporting passed. Homing was not run.")
            return 0
        ...
        if missing:
            raise ChessGantryError("M119 did not report " + ", ".join(missing))
        position = None
        for line in positions.responses:
            ...
            if match:
                ...
                break
        if position is None:
            raise ChessGantryError("M114 did not return parseable X/Y/Z coordinates")
        expected_position = (0.0, 350.0, 350.0)
        if any(
        ...
        print("Firmware homing passed: X0 Y350 Z350.")
    return 0
