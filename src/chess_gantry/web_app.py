...
(async()=>{await ports();await status();await autoConnect();await board();await loadOperations();await taskStatus();await liveStatus();setInterval(()=>{if(!busy)status();taskStatus();liveStatus()},750);setInterval(readPosition,750)})();
</script></body></html>"""
