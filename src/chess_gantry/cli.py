...
def _parser() -> ArgumentParser:
    ...
    run.add_argument("move_json", help="move-delta JSON file (e.g. examples/move_e2_e4.json)")
    run.add_argument(
        "--confirm-motion",
        action="store_true",
    ...
    run.add_argument("--summary-json", action="store_true", help="print plan summary as JSON")
    ...
    uci.add_argument("--output", "-o", help="write move JSON to this file instead of stdout")
    lichess = commands.add_parser(
        "lichess-event",
        help="convert a move envelope from services/lichess_stream into gantry JSON and G-code",
    )
    lichess.add_argument("event_json", help="saved type=move WebSocket envelope from the Lichess stream")
    lichess.add_argument("--move-output", help="write converted gantry move JSON to this file")
    lichess.add_argument("--gcode-output", help="write planned G-code to this file")
    watch = commands.add_parser(
        "lichess-watch", help="subscribe to the upstream Lichess API and plan or execute moves"
    )
    watch.add_argument("game_id", help="Lichess game id")
    watch.add_argument("--stream-url", default="ws://127.0.0.1:8010", help="upstream stream service URL")
    watch.add_argument("--output-dir", default="data/lichess", help="directory for generated event JSON and G-code")
    watch.add_argument("--execute", action="store_true", help="execute every received move instead of planning only")
    watch.add_argument("--confirm-motion", action="store_true", help="required with --execute")
    pgn = commands.add_parser("lichess-pgn", help="fetch a Lichess game and dry-run all recorded moves to JSON and G-code")
    pgn.add_argument("game_id", help="Lichess game id")
    pgn.add_argument("--output-dir", default="data/lichess", help="directory for generated event JSON and G-code")
    follow = commands.add_parser("lichess-follow", help="poll Lichess PGN and automatically create JSON/G-code for new moves")
    follow.add_argument("game_id", help="Lichess game id")
    follow.add_argument("--output-dir", default="data/lichess", help="directory for generated move JSON and G-code")
    follow.add_argument("--session", help="persistent follow-session JSON path")
    follow.add_argument("--interval", type=float, default=5.0, help="PGN polling interval in seconds")
    follow.add_argument("--once", action="store_true", help="poll once, generate available new moves, then exit")
    follow.add_argument("--reset-session", action="store_true", help="forget emitted move history and recreate files")
    follow.add_argument("--execute", action="store_true", help="stream new moves to Marlin and commit board state")
    follow.add_argument("--execute-existing", action="store_true", help="with --execute, also stream already dry-run recorded moves")
    follow.add_argument("--confirm-motion", action="store_true", help="required with --execute")
    follow.add_argument(
        "--obstacle-keepout-mm",
        type=float,
    ...
    diagnose.add_argument("--port", help="override serial port; omit to use config/auto-detection")
    ...
    web.add_argument("--host", default="127.0.0.1", help="bind address (default: local only)")
    web.add_argument("--web-port", type=int, default=8000, help="browser port (default: 8000)")
    web.add_argument("--no-browser", action="store_true", help="do not open a browser automatically")
    web.add_argument("--demo", action="store_true", help="run with a simulated Marlin controller")
    web.add_argument(
        "--allow-network",
        action="store_true",
...
def run(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if args.command == "lichess-follow" and args.obstacle_keepout_mm is not None:
            if args.execute:
                parser.error("--obstacle-keepout-mm is dry-run-only and cannot be combined with --execute")
            if args.obstacle_keepout_mm <= 0:
                parser.error("--obstacle-keepout-mm must be positive")
            config = replace(
                config,
                planner=replace(config.planner, obstacle_keepout_mm=args.obstacle_keepout_mm),
            )
            print(
        ...
        if args.command == "lichess-event":
            ...
            with service.store.locked():
                ...
                move = stream_event_to_move(
                    read_json(Path(args.event_json)), config.board.width, config.board.height, state
                )
                plan = service.plan(move, state)
            if args.move_output:
                output = Path(args.move_output)
                output.parent.mkdir(parents=True, exist_ok=True)
                output.write_text(json.dumps(move.to_dict(), indent=2, sort_keys=True) + "\n", encoding="ascii")
                print(f"Wrote move JSON to {output}.")
        ...
        if args.command == "lichess-watch":
            ...
            asyncio.run(
                watch_game(
                    service, args.stream_url, args.game_id, Path(args.output_dir), execute=args.execute
                )
            )
            return 0
        ...
        if args.command == "lichess-pgn":
            if service.journal.exists():
                raise PendingTransactionError(f"pending transaction exists at {service.journal.path}; reconcile it first")
            ...
            for move in pgn_moves(args.game_id, pgn, state):
                ...
                (output_dir / f"{move.event_id}.gcode").write_text(plan.program.text(), encoding="ascii")
                count += 1
        ...
        if args.command == "lichess-follow":
            ...
            output_dir = Path(args.output_dir)
            session_path = Path(args.session) if args.session else output_dir / f"{args.game_id}.session.json"
            follow_game(
                service,
                args.game_id,
                output_dir,
        ...
        if args.command == "run":
            ...
            if not args.confirm_motion:
                ...
                if args.output:
                    ...
                    output.parent.mkdir(parents=True, exist_ok=True)
                    output.write_text(plan.program.text(), encoding="ascii")
                    print(f"Wrote {len(plan.program.commands)} commands to {output}.", file=sys.stderr)
                else:
                    print(plan.program.text(), end="")
                return 0
            plan = service.execute(move)
