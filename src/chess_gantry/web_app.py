...
<body><main>
<header><div><h1>Chess Gantry Controller</h1><p class="subtitle">One local interface for serial diagnostics, coupled outer X/Y motion, independent inner E motion, and validated chess move JSON.</p></div><div id="pill" class="pill">Starting…</div></header>
<section class="grid">
...
<div class="card"><h2>2. Machine state</h2><div class="metrics"><div class="metric"><span>Inner E</span><strong id="xread">—</strong></div><div class="metric"><span>Outer position</span><strong id="yread">—</strong></div><div class="metric"><span>Initialized</span><strong id="homed">No</strong></div><div class="metric"><span>Board rev.</span><strong id="revision">—</strong></div></div><div class="actions"><button id="endstops">Check endstops</button><button id="home" class="primary">Set current XYE origin</button><button id="stop" class="danger">Emergency stop</button></div><div class="notice">Physical X/Y are mechanically mirrored outer motors: moving away from the switches decreases X while increasing Y. E is the independent inner motor. No homing occurs.</div></div>
